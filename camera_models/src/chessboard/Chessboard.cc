  ...
  for (int k = 0; k < 6; ++k) {
    for (int dilations = minDilations; dilations <= maxDilations; ++dilations) {
      ...
      if (flags & cv::CALIB_CB_ADAPTIVE_THRESH) {
        int blockSize = lround(prevSqrSize == 0 ? std::min(img.cols, img.rows) *
                                                      (k % 2 == 0 ? 0.2 : 0.1)
                                                : prevSqrSize * 2) |
                        1;
      ...
      cv::Mat kernel1 = cv::getStructuringElement(
          CV_SHAPE_CROSS, cv::Size(3, 3), cv::Point(1, 1));
      cv::Mat kernel2 = cv::getStructuringElement(CV_SHAPE_RECT, cv::Size(3, 3),
                                                  cv::Point(1, 1));
...
void Chessboard::labelQuadGroup(std::vector<ChessboardQuadPtr> &quadGroup,
        ...
        for (int j = 0; j < 4; j++) {
          ...
          if (quad->neighbors[j]) {
            ...
            if (quadNeighbor->labeled) {
              ...
              quad->corners[(j + 1) % 4]->column = conCorner->column -
                                                   conCornerCW2->column +
                                                   conCornerCW3->column;
              ...
              quad->corners[(j + 3) % 4]->column = conCorner->column -
                                                   conCornerCW2->column +
                                                   conCornerCW1->column;
  ...
  // The only time we can make shure that the target pattern size is reached in
  // a given dimension, is when the larger side has reached the target size in
  // the maximal direction, or if the larger side is larger than the smaller
  // target size and the smaller side equals the smaller target size
  ...
  if ((largerDimPattern + 1) == max_column - min_column) {
    ...
    // reached Therefore border column corners do not need a neighbor anymore Go
    // through all corners
    for (int k = 0; k < (int)quadGroup.size(); ++k) {
      ChessboardQuadPtr &q = quadGroup.at(k);
  ...
  if ((flagSmallerDim1 == true && flagSmallerDim2 == false)) {
    // Larger target pattern size is in column direction, check wheter smaller
    // target pattern size is reached in row direction
...
void Chessboard::generateQuads(std::vector<ChessboardQuadPtr> &quads,
  ...
  for (size_t i = 0; i < contours.size(); ++i) {
    ...
    // "first run" is set to "true". This guarantees, that only "nice" squares
    // are detected. During PART 2, we allow the polygonal approximation
    // function below to approximate more freely, which can result in recognized
    // "squares" that are in reality multiple blurred and sticked together
    // squares.
...
bool Chessboard::checkQuadGroup(std::vector<ChessboardQuadPtr> &quads,
  ...
  // If in a given direction the target pattern size is reached, we know exactly
  // how the checkerboard is oriented. Else we need to prepare enough "dummy"
  // corners for the worst case.
  for (size_t i = 0; i < quads.size(); ++i) {
    ChessboardQuadPtr &q = quads.at(i);
...
bool Chessboard::checkChessboard(const cv::Mat &image,
  ...
  for (float threshLevel = blackLevel; threshLevel < whiteLevel && !result;
    ...
    // Initialize contour retrieving routine
    cv::findContours(thresh, contours, hierarchy, CV_RETR_CCOMP,
                     CV_CHAIN_APPROX_SIMPLE);
    ...
    for (size_t i = 0; i < quads.size(); ++i) {
      ...
      if (j + 1 > min_quads_count + i) {
        ...
        const int blackCount = lroundf(ceilf(patternSize.width / 2.0f) *
                                       ceilf(patternSize.height / 2.0f));
        const int whiteCount = lroundf(floorf(patternSize.width / 2.0f) *
