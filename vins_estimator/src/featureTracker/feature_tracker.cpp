  ...
  */
  ...
  // 把左目的点在右目上找到，然后计算右目上的像素速度。
  {
    ...
    if (!cur_pts.empty()) {
      ...
      nextPts
      可以作为输入，也可以作为输出。包含输入特征在第二幅图像中计算出的新位置的二维点（单精度浮点坐标）的输出vector。当使用OPTFLOW_USE_INITIAL_FLOW
      标志时，nextPts的vector必须与input的大小相同。 status
      输出状态vector(类型：unsigned
      chars)。如果找到了对应特征的流，则将向量的每个元素设置为1；否则，置0。 err
      误差输出vector。vector的每个元素被设置为对应特征的误差，可以在flags参数中设置误差度量的类型；如果没有找到流，则未定义误差（使用status参数来查找此类情况）。
      winSize	每级金字塔的搜索窗口大小。
      maxLevel
      基于最大金字塔层次数。如果设置为0，则不使用金字塔（单级）；如果设置为1，则使用两个级别，等等。如果金字塔被传递到input，那么算法使用的级别与金字塔同级别但不大于MaxLevel。
      criteria
      指定迭代搜索算法的终止准则（在指定的最大迭代次数标准值（criteria.maxCount）之后，或者当搜索窗口移动小于criteria.epsilon。）
      flags 操作标志，可选参数：
      OPTFLOW_USE_INITIAL_FLOW：使用初始估计，存储在nextPts中；如果未设置标志，则将prevPts复制到nextPts并被视为初始估计。
      OPTFLOW_LK_GET_MIN_EIGENVALS：使用最小本征值作为误差度量（见minEigThreshold描述）；如果未设置标志，则将原始周围的一小部分和移动的点之间的
      L1 距离除以窗口中的像素数，作为误差度量。 minEigThreshold
      算法所计算的光流方程的2x2标准矩阵的最小本征值（该矩阵称为[Bouguet00]中的空间梯度矩阵）÷
      窗口中的像素数。如果该值小于MinEigThreshold，则过滤掉相应的特征，相应的流也不进行处理。因此可以移除不好的点并提升性能。
    */
...
map<int, vector<pair<int, Eigen::Matrix<double, 7, 1>>>>
FeatureTracker::trackFeature(double _cur_time,
                             const vector<cv::Point2f> &_features0,
                             const vector<cv::Point2f> &_features1) {
  TicToc t_r;
  cur_time = _cur_time;
  ...
  */
  ...
  if (1) {
    ...
    int n_max_cnt =
        MAX_CNT -
        static_cast<int>(cur_pts.size()); // 特征恒定，只有在第一次才会发生
...
void FeatureTracker::drawTrack(const cv::Mat &imLeft, const cv::Mat &imRight,
  ...
  for (size_t j = 0; j < curLeftPts.size(); j++) {
    ...
    cv::circle(imTrack, curLeftPts[j], 2,
               cv::Scalar(255 * (1 - len), 0, 255 * len), 2); // 颜色为bgr顺序
  }
  if (!imRight.empty() && stereo_cam) {
